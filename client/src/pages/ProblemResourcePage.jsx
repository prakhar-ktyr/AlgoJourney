...
export default function ProblemResourcePage() {
  ...
  return (
    <div
      className={`${readerActive ? `min-h-screen ${theme.bg} ${theme.text} transition-colors duration-300` : ""}`}
    >
      <div
        className={`${readerActive ? "max-w-3xl" : "max-w-4xl"} mx-auto px-4 sm:px-6 py-12 ${readerActive ? `${fontFamily.className} ${fontSize.className}` : ""}`}
      >
        {!readerActive && (
          <Link
            to="/dsa-sheet"
        ...
        <header className="mb-6">
          ...
          <div className="flex flex-wrap items-baseline gap-3">
            <h1
              className={`${readerActive ? "text-2xl" : "text-3xl"} font-bold ${readerActive ? "" : "text-white"}`}
            >
              {problem.title}
            </h1>
        ...
        <div
          className={`flex flex-wrap items-center gap-2 mb-8 sticky top-0 z-10 ${readerActive ? `${theme.bg}/85` : "bg-gray-950/85"} backdrop-blur py-3 border-b ${readerActive ? "border-current/10" : "border-gray-800"}`}
          data-testid="language-selector"
        >
          <label
            htmlFor="language-select"
            className={`text-sm ${readerActive ? "opacity-70" : "text-gray-400"} mr-1`}
          >
            Language:
          </label>
          <select
            ...
            className={`${readerActive ? `${theme.bg} border-current/20` : "bg-gray-900 border-gray-700 text-white"} border text-sm rounded px-3 py-1.5 cursor-pointer focus:outline-none focus:border-indigo-500`}
